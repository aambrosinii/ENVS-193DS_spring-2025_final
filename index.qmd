...
sst_filtered <- sst_clean[sst_clean$year %in% 2018:2023, ]
sst_filtered$year <- factor(sst_filtered$year)
...
ggplot(sst_filtered, aes(x = month, y = mean_monthly_sst, group = year, color = year)) +
  ...
  theme(
    ...
    panel.border = element_rect(fill = NA, color = "black", linewidth = 0.8),  # Border
 legend.position.inside = c(0.1, 0.8), # Legend inside plot panel (x, y from 0 to 1)
